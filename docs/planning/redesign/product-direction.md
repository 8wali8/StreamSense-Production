...
- `/sessions/:sessionId` session report, a one-pager: always visible are the title row with the risk pill, five headline numbers (on screen, mentions, sentiment, viewers, media value), the timeline with the selected moment, and the best and weakest moment as one line each. Below that, always open and laid out two across so the whole page reads as one summary: how the value was estimated (with the private fee block and direct response), every brand mention, why risk is what it is (the named weighted factors), and the stream overall. No collapsed sections: the reader should see everything at once and dive in by reading, not by clicking. S2, S3, S4, S6. This is the product; build first among customer pages.
