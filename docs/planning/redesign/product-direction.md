...
- `/sessions/:sessionId` session report, one page, numbers first: title and risk pill; four tiles (on screen with share of stream, mentions with positive share, average and peak viewers, media value with the fee multiple); the timeline with the selected moment and a VOD link; best and weakest moment as one line each; one strip of six numbers (logo value, host-read value, fee, link clicks, chat command uses, channel point redemptions). No explanatory text on the page. Four links at the bottom open the detail: how value is estimated, all mentions, risk factors, full stream stats. S2, S3, S4, S6. This is the product; build first among customer pages.
