...
2. Enter an average viewer figure and click Import. The row shows "Importing · N%" from the capture status; chat lines appear in the session's report within a minute, frames and transcripts follow at roughly the recording's length divided by the sampling speed of the machine.
