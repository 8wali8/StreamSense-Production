...
- **Idempotent replays**: video-service derives the detection id from the frame id and sentiment-service the sentiment ids from the chat event or segment id (name-based UUIDs), and the capture import names frames and segments by offset, so a message or frame processed twice (a retried delivery, a resumed import) is counted once. A failed capture import (twenty consecutive misses, with a growing pause between them, since Twitch throttles bursts of HLS seeks) resumes from its last offset when asked again; the deal page offers Resume.
