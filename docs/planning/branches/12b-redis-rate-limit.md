...
- **Wiring**: `spring.data.redis.host/port` (`REDIS_HOST`, `REDIS_PORT`, default `redis`) in config-repo; the Compose gateway waits for the healthy `redis` service; the Kubernetes gateway gains a `wait-for-redis` init container in the same non-root shape as the others. The gateway's readiness probe does not include Redis, matching fail-open.
