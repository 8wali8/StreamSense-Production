...
**Failures are classified by type, not by class name.** The capture loop previously did `if "Kafka" in exc.__class__.__name__`. It now catches `kafka.errors.KafkaError` (state `DEGRADED_KAFKA`), then `botocore` `BotoCoreError`/`ClientError` and `OSError` (state `DEGRADED_STORAGE`), and finally a labelled `except Exception` at the worker boundary that logs a full traceback and counts the frame as `unexpected`. The transcript path gets the same split. The `BLE001` per-file ignore stays only for those two boundary handlers.
...
**Readiness reflects the workers.** `/live` (always 200), `/ready` (503 `starting` before the lifespan; with capture disabled `ready`; with capture enabled 200 only while every configured channel's thread is alive, otherwise 503 `degraded` with the counts), `/health` kept for existing callers with a `ready` flag. Kubernetes readiness uses `/ready` and liveness `/live`; the Compose healthcheck uses `/ready` so dependants wait for the workers. The frame-proxy endpoint reuses the storage backend's boto3 client instead of building one per request, and returns 409 when the runtime is not on S3 storage.
