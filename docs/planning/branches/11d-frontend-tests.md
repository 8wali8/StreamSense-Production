...
- **Unhandled requests fail the test that made them.** `onUnhandledRequest: "error"` only rejects the request, and a component that swallows the network error (Apollo, the polling hook) would let the test pass; `setup.ts` also records `request:unhandled` events and throws in `afterEach`.
- **Fixtures are typed from the generated operation types** (`RecentSentimentQuery["recentSentiment"][number] & { __typename }` and so on), so a renamed field or a changed nullability in the schema is a compile error in the fixtures instead of a test exercising a response the gateway cannot return.
- **The analytics fixture is typed too**: `streamAnalytics()` returns `WithTypenames<StreamAnalyticsQuery>`, the generated shape with an optional `__typename` on every object. **The roster test records the video-capture switch** and asserts it is sent on the initial streamer change and not again when the same streamer is reloaded.
- **The replay console test is scoped to the raw transcript feed**, because the sponsor sentiment feed renders the same segment text and a page-wide search would stay green if the raw feed lost every line.
