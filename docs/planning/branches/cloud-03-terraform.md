...
- **`.gitignore`**: Terraform state, variable files, the provider directory, and `crash.log` are ignored; the lock file and the `.example` are not.
...
| Validate | `terraform init -backend=false` then `terraform validate` | "The configuration is valid"; provider `google` 8.2.0 locked |
...
1. `gcloud auth application-default login`, then in `terraform/`: copy the example to `terraform.tfvars`, set `project_id` and your `/32` in `allowed_ssh_cidrs`, and run `terraform init`, `terraform plan` (expect 10 resources), `terraform apply`.
