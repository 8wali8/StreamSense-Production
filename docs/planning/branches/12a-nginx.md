...
- **`/ml/segment` goes through the gateway, not straight to ml-engine.** Review found that the direct proxy bypassed the gateway's auth and rate-limit filters while `/ml/segment` accepts `file://` references, so an unauthenticated request could have read an unbounded local path on the ML pod. The gateway gains an `ml-engine-segment` route (`POST /ml/segment` to `ML_ENGINE_URL`, 180 s response timeout for the SAM cold start), `/ml/**` joins the auth filter's protected paths, and a `ml-segment` rate-limit rule allows 30 requests per minute per client; nginx and the Vite dev proxy forward `/ml/segment` to the gateway, and the frontend drops its separate ML origin setting. Independently, ml-engine now reads only regular files and bounds every frame read (file and S3) by `STREAMSENSE_FRAME_MAX_BYTES`, default 32 MiB, with tests.
