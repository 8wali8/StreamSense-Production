...
**CI** (`schema-compat` job): `tools/schema/check_compat.py` diffs every schema against the pull request's base branch (or the previous commit on a push to `main`) and fails on a new required property, a removed property, a narrowed type, a lost enum value, an enum on a previously free property, or any validation keyword that is added or tightened (`minimum`/`maximum` and their exclusive forms, `minLength`/`maxLength`, `minItems`/`maxItems`, `minProperties`/`maxProperties`, `pattern`, `format`, `const`, `multipleOf`, `uniqueItems`), recursing into nested `properties` and `items`; it exits 2 when the base ref does not resolve, so an unfetched ref cannot make every schema look new; the root node is compared like any other (a root type change or a root bound counts); a property that had no `type` gaining one is breaking unless its old `const`/`enum` already confined it to that type; and a reviewed `docs/schemas/compat-exceptions.txt` can list a finding verbatim to accept a deliberate tightening (empty today); applicator keywords (`not`, `allOf`, `anyOf`, `oneOf`, `if`/`then`/`else`, `contains`, `propertyNames`, `patternProperties`, `dependentRequired`, `dependentSchemas`, `unevaluatedProperties`, `unevaluatedItems`) that are added or changed fail closed because the script cannot prove them compatible; `integer` widening to `number`, `uniqueItems` relaxing, and a `multipleOf` that the old step divides evenly are accepted; enum values compare with JSON semantics; `items: false` on a previously unconstrained array is breaking; it also lists the base ref's schemas, so a schema that disappears fails unless the script's `RENAMED` map declares its new name, in which case the renamed file is compared against the old one (the four renames in this branch are declared there); `additionalProperties` turning false (or restrictive) on any node is breaking too; this branch's four closures are listed in `docs/schemas/compat-exceptions.txt` with the reason (every producer is in this repository and contract-tested against the closed schema). `docs/schemas/**` also counts as a shared Java change, so every Java module's contract tests run when a schema moves.
...
- Schemas and contract tests for the remaining ml-engine payloads (sponsor relevance, sponsor detection, segmentation, transcription); today only the two sentiment payloads are covered, and the schemas README says so.
