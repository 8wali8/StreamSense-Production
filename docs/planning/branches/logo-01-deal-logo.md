...
**analytics-service.**
- `V10__deal_logos.sql` (numbered V9 when the branch merged, alongside the imports branch's own V9; renumbered in `fix/02-duplicate-v9`): one row per logo on a deal (object key, content type, sha256, width, height, size, upload time), cascading with the deal.
