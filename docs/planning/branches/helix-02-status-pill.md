...
**Gateway.** Nothing: the existing `/api/analytics/**` route forwards the path, and the channel-scope filter passes paths it does not recognise. Reads on analytics routes are open to streamer tokens, which is why the body carries no logins.
...
- **The watched logins** are not exposed (see above). If they are ever wanted on the pill, the route needs the operator role first.
...
| Route | `HelixStatusTest` | `enabled: false` with the poller off (MockMvc, the test context); the controller returns the poller's snapshot when there is one |
