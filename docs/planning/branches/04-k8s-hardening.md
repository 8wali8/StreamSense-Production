...
**Stateful data moves to PersistentVolumeClaims.** `k8s/platform/storage.yaml` declares `postgres-data` (5 Gi), `minio-data` (10 Gi), `ml-engine-whisper-cache` (2 Gi), and `ml-engine-relevance-cache` (1 Gi). Postgres, MinIO, and ml-engine mount them instead of `emptyDir` and use `strategy: Recreate`, which a single-replica Deployment with a ReadWriteOnce claim needs. kind's default StorageClass provisions the claims dynamically.
...
**Housekeeping**: the topics Job gets `ttlSecondsAfterFinished: 600`; CI, `docs/kubernetes-kind.md`, CLAUDE.md, AGENTS.md, and `docs/current-state.md` describe the root entry point and the new rules.
