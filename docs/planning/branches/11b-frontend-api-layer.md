...
- **Same-origin is the supported setup.** `VITE_API_BASE_URL` and `VITE_ML_BASE_URL` are documented as needing an edge that adds CORS, since neither the gateway nor ml-engine sends CORS headers; the Docker image and the dev server proxy everything on one origin. The client's default deadline is 15 s, above the gateway's 10 s response timeout, so a stalled downstream arrives as the gateway's problem detail instead of the browser's `AbortError`; segmentation gets 180 s because the SAM backend's first call downloads and initialises the model.
...
- **`src/lib/api-client.ts`**: `apiRequest` / `apiFetch<T>` / `apiSend` / `apiUrl`. Query parameters go in `params` and the base URL is applied exactly once inside the client (`apiUrl` is for absolute URLs used outside it, such as an `<img src>`); a call can pass `baseUrl` for an origin other than the gateway, which `/ml/segment` uses with `VITE_ML_BASE_URL`, because the gateway does not route `/ml`. Every call gets the base URL, `Accept`, `Content-Type` when there is a body, the same `Authorization: Bearer` header as GraphQL (`src/lib/auth-token.ts`, shared with the Apollo links), and a timeout signal (15 s default, above the gateway's 10 s response timeout; 180 s for segmentation; an `AbortController` plus timer when `AbortSignal.timeout` is unavailable). A non-2xx response throws `ApiError` with `status`, `path`, and the parsed `problem` (`detail`, `service`, `correlationId`, `errors[]`), so the message reads `/api/chat/twitch/channels returned 409: Twitch chat ingestion is disabled`. Test doubles without headers still work.
...
- **Vite dev proxy** (`vite.config.ts`): port 5173 (Vite's default; 3000 is the Compose frontend container, so `make up` and `npm run dev` can run side by side), `/graphql` (with WebSocket upgrade) and `/api` to `VITE_DEV_API_TARGET` (default `http://localhost:8080`), `/ml` to `VITE_DEV_ML_TARGET` (default `http://localhost:8000`), read through `loadEnv` so `.env.local` works. `frontend/.env.example` documents the three variables; `frontend/README.md` is now a real README.
...
| Dev proxy routes | two stub JSON servers on 8080 and 8000, `npx vite --port 5173`, `curl localhost:5173/api/chat/twitch/status` and `/ml/live` | each answered by the right upstream (`"port": 8080` / `"port": 8000`); `/` serves `index.html` |
| Docker image | `docker build frontend/` | builds |

## Manual checks for the reviewer

1. `make up`, then in `frontend/` run `npm run dev` and open `http://localhost:5173`: the console loads data and subscriptions stream over the proxied WebSocket, with no CORS errors in the browser console.
