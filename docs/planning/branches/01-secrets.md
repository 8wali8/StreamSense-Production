...
**Kubernetes** builds a `streamsense-secrets` Secret in the `streamsense` namespace with `secretGenerator` from git-ignored `k8s/secrets/streamsense.env` (`disableNameSuffixHash` so manifests can reference a stable name). postgres, minio, grafana, ml-engine, video-capture-service, sentiment-service, video-service, analytics-service, and api-gateway read their credentials through `secretKeyRef`. The gateway previously referenced a secret named `api-gateway-auth` that nothing created; it now uses the generated one.

**Tooling and docs**: `make secrets` creates the real files from the committed `*.example` files and is a prerequisite of `up`, `up-fast`, and `smoke-e2e`; `tools/start-stack.ps1` does the same copy; CI creates the files before `docker compose config` and before `kubectl kustomize`; `secrets/README.md`, `docs/howtorun.md`, `docs/kubernetes-kind.md`, `CLAUDE.md`, `AGENTS.md`, and `README.md` describe the new flow. `.gitignore` excludes `secrets/*` except the README and examples, and `k8s/secrets/*.env`.
...
- Example values are the historical development defaults so an existing local stack keeps working after `make secrets`. Anyone exposing the stack must change them; the README says so.
...
|---|---|---|
| Local secret files | `for f in secrets/*.example; do cp -n "$f" "${f%.example}"; done; cp -n k8s/secrets/streamsense.env.example k8s/secrets/streamsense.env` | created; `git check-ignore` confirms both locations are ignored |
...
| Generated Secret | grep of the rendered output | `streamsense-secrets` in namespace `streamsense`, 5 keys, referenced 13 times |
| video-capture-service tests | `pytest src/test/python` in `python:3.11-slim` | 23 passed (4 new) |
| ml-engine tests | `pytest src/test/python/test_frame_store.py` in `python:3.11-slim` | 5 passed (3 new); the full suite needs torch and was not run locally, CI runs it |
...
6. On kind: `kubectl apply -k k8s` after creating `k8s/secrets/streamsense.env`; `kubectl -n streamsense get secret streamsense-secrets` shows 5 keys.
