...
| 01 | `cloud/01-image-publish` | A `publish-images` job in `ci.yml`: runs on pushes to `main` after `ci-ok` succeeds and on `workflow_dispatch`; packages the Java reactor once; logs into GHCR with the workflow token (`packages: write` on this job only); builds and pushes the eleven images with the Actions build cache, tagged `<sha>` and, on `main`, `main`. The packages inherit the public repository's visibility, so the VM pulls anonymously. | 00 | half day | low: inert for every other job | A dispatched run from the branch pushes eleven images; `docker pull` of one by SHA works anonymously |
