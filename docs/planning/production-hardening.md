...
- **Local verification runs in containers.** Java builds and tests run in `maven:3.9-eclipse-temurin-21` and Python tests in `python:3.11-slim`, so results match CI regardless of what is installed on the contributor's machine. Frontend runs on local Node. Manifests are rendered with `kubectl kustomize k8s`.
...
|---|---|---|
| Render | `docker compose config -q` and `kubectl kustomize k8s > /dev/null`. From branch 01 on, run `make secrets` first (it copies the committed `*.example` secret files into the git-ignored locations; CI does the same copy before rendering). Branches before 01 need no secret files. If `k8s/config/grafana-config.yaml` changed, also run the same JSON check CI runs: parse every entry of the `grafana-dashboards` ConfigMap with `json.loads` (the step is in `.github/workflows/ci.yml`), because YAML rendering treats the embedded dashboards as opaque text | Compose, Kubernetes, or config-repo changed |
...
| Python | `docker run --rm -v "$PWD/<service>:/app" -w /app python:3.11-slim sh -c "pip install -q -r requirements.txt ruff==0.16.3 && ruff check src/main/python src/test/python && PYTHONPATH=src/main/python pytest -q src/test/python"` (or, once branch 02c lands, `docker run --rm -v "$PWD/<service>:/app" -w /app ghcr.io/astral-sh/uv:0.12.9-python3.11-bookworm-slim sh -c "uv sync --locked && uv run ruff check src/main/python src/test/python && uv run pytest"`; the image carries the same pinned uv as the Dockerfiles and CI, and `--locked` makes a stale `uv.lock` fail here the same way it fails there). The Ruff step matches the CI lint gate, so a branch cannot record a passing Python rung and still fail CI on lint | Any Python service changed |
...
| 05 | `hardening/05-kafka-kraft` | 5 | Single-node KRaft in Compose and a Kafka StatefulSet with PVC in Kubernetes; remove ZooKeeper; update kafka-ui and the topics init; update the CI docker-smoke service list so it no longer starts a ZooKeeper service | 04 | high: message broker replaced | Topics created; replay smoke passes; kafka-exporter still scrapes |
| 06a | `hardening/06a-ml-engine-lifecycle` | 6 | `create_app()` with lifespan; pydantic-settings; backend registry loaded once with a lock; SAM no longer rebuilt per request; transcribe off the event loop; `/ml/live`, `/ml/ready`, `/ml/info`; force-failure as a router dependency; typed exception handlers; Prometheus `/metrics`; Kubernetes liveness moves to `/ml/live` and readiness and the startup probe to `/ml/ready`, while `/ml/health` stays as the Compose healthcheck, init-container gate, and smoke-tool endpoint so nothing that calls it breaks | 05 | medium | Inference endpoints behave identically; health now reflects model readiness |
...
| 07 | `hardening/07-parent-pom` | 7 | Root `pom.xml` with modules, Spring Cloud and Resilience4j BOMs, enforcer, Spotless (check only, no reformat yet), jacoco, build-info and git-commit-id plugins; per-service POMs shrink to their own dependencies | 06b | medium: build graph changes, versions unchanged | `mvn -B -ntp clean verify` at the root builds all modules through one reactor (enforcer, tests, JaCoCo); `mvn -f <service>/pom.xml` still works; CI matrix unchanged |
...
| 12b | `hardening/12b-redis-rate-limit` | 11 | Gateway `RequestRateLimiter` with `RedisRateLimiter` and the existing key resolver logic; remove the in-memory limiter; Redis connection settings in `config-server/config-repo/api-gateway.yml` and its Kubernetes mirror, and the gateway depends on the `redis` service in Compose; the rate-limit integration tests start an embedded Redis in the JVM so they run in the standalone Maven container and in the CI matrix without Docker or a provisioned service | main | medium | Existing rate-limit integration tests pass against a real Redis; the gateway rate-limits under Compose and Kubernetes |
...
Branches 01 through 05 are the security and operability tier and should merge first. Branches 06 through 10 are the correctness tier. Branches 11 through 14 are structure and hygiene. The independent groups (the 11a to 11d chain, 12a, 12b, 13, 14) can be reordered relative to each other, but 11a through 11d keep their listed order because each is stacked on the previous one.
