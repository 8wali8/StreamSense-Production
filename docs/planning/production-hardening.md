...
| Python | `docker run --rm -v "$PWD/<service>:/app" -w /app python:3.11-slim sh -c "pip install -q -r requirements.txt ruff==0.16.3 && ruff check src/main/python src/test/python && PYTHONPATH=src/main/python pytest -q src/test/python"` (or `uv sync --locked && uv run ruff check src/main/python src/test/python && uv run pytest` once branch 02c lands; `--locked` makes a stale `uv.lock` fail here the same way it fails in the Dockerfiles and CI). The Ruff step matches the CI lint gate, so a branch cannot record a passing Python rung and still fail CI on lint | Any Python service changed |
...
| 05 | `hardening/05-kafka-kraft` | 5 | Single-node KRaft in Compose and a Kafka StatefulSet with PVC in Kubernetes; remove ZooKeeper; update kafka-ui and the topics init; update the CI docker-smoke service list so it no longer starts a ZooKeeper service | 04 + 02a + 02c | high: message broker replaced | Topics created; replay smoke passes; kafka-exporter still scrapes |
...
| 12b | `hardening/12b-redis-rate-limit` | 11 | Gateway `RequestRateLimiter` with `RedisRateLimiter` and the existing key resolver logic; remove the in-memory limiter; the rate-limit integration tests start their own Redis (Testcontainers) so they run in the standalone Maven container and in the CI matrix without a provisioned service | main | medium | Existing rate-limit integration tests pass against a real Redis |
