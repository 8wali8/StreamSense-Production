...
| `STREAMSENSE_FRAME_STORAGE_ACCESS_KEY` | minio (root user), ml-engine, video-capture-service, analytics-service (deal logos) | The Python services read `<NAME>_FILE` first, then `<NAME>`. MinIO requires 3 to 20 characters; the generator writes 16. |
| `STREAMSENSE_FRAME_STORAGE_SECRET_KEY` | minio (root password), ml-engine, video-capture-service, analytics-service (deal logos) | Same as above. MinIO requires 8 to 40 characters; the generator writes 32. |
