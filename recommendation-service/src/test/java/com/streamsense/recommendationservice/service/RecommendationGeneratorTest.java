...
class RecommendationGeneratorTest {
    ...
    private final RecommendationGenerator generator = new RecommendationGenerator(
            Clock.fixed(Instant.parse("2026-04-12T03:00:00Z"), ZoneOffset.UTC));
    ...
    void generatesDeterministicRecommendationsForSameInputs() {
        ...
        assertThat(first).extracting(Recommendation::category)
    ...
    void sponsorBoostVariantRaisesSponsorRecommendationAboveBalanced() {
        ...
        Recommendation balancedSponsorRecommendation = findCategory(generator.generate(
                "test",
                3,
                sentimentSignals,
                sponsorSignals,
                ...
                "SPONSOR_ALIGNMENT");
        Recommendation sponsorBoostRecommendation = findCategory(generator.generate(
                "test",
                3,
                sentimentSignals,
                sponsorSignals,
    ...
    private List<SentimentSignal> positiveSentimentSignals() {
        return List.of(
                new SentimentSignal("sent-1", "src-1", "test", "u1", "great stream", 1710000000000L, 1710000000500L, "POSITIVE", 0.82d, "stub-v1"),
                new SentimentSignal("sent-2", "src-2", "test", "u2", "love this", 1710000001000L, 1710000001500L, "POSITIVE", 0.73d, "stub-v1"),
                new SentimentSignal("sent-3", "src-3", "test", "u3", "pretty good", 1710000002000L, 1710000002500L, "NEUTRAL", 0.12d, "stub-v1"));
    }

    private List<SponsorSignal> sponsorSignals() {
        return List.of(
                new SponsorSignal("det-1", "frame-1", "test", "frames/1.png", 1.0d, 1710000000000L, 1710000000500L, "Nike", 0.91d, "stub-v1", 0.1d, 0.2d, 0.3d, 0.4d),
                new SponsorSignal("det-2", "frame-2", "test", "frames/2.png", 2.0d, 1710000001000L, 1710000001500L, "Nike", 0.88d, "stub-v1", 0.1d, 0.2d, 0.3d, 0.4d),
                new SponsorSignal("det-3", "frame-3", "test", "frames/3.png", 3.0d, 1710000002000L, 1710000002500L, "Prime", 0.67d, "stub-v1", 0.1d, 0.2d, 0.3d, 0.4d));
    }
}
