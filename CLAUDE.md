...
`docker-compose.prod.yml` is the overlay for the hosted demo (`docs/planning/cloud-hosting.md`): layered over the base file it pulls the GHCR images at `STREAMSENSE_IMAGE_TAG` (default `main`) instead of building, puts a Caddy edge on ports 80 and 443 in front of the console (HTTPS with an automatic Let's Encrypt certificate when `STREAMSENSE_DOMAIN` is set, plain HTTP on the address otherwise; `tools/deploy/caddy/Caddyfile`) with the admin UIs on `127.0.0.1`, sets memory limits copied from the Kubernetes manifests, restarts containers with the machine, rotates logs, and turns gateway auth on with GraphiQL off and two trusted proxy hops. Twitch settings, the domain, and the tag reach it through `docker compose --env-file <file>`, which feeds the base file's `${TWITCH_*}` interpolation and the overlay's own placeholders. CI renders the pair in the smoke job; run `docker compose -f docker-compose.yml -f docker-compose.prod.yml config -q` after touching either file.
...
CI (`.github/workflows/ci.yml`) uses Java 21, Python 3.11, Node 20. Its Java matrix covers all seven Java services, and it also tests video-capture-service. `make test` is not identical to CI: for frontend it runs only `lint` + `build` and skips Vitest, while CI runs Vitest too. If you touch `k8s/` or `config-server/config-repo/`, run `kubectl kustomize .` from the repo root — CI validates that plus the JSON embedded in `k8s/config/grafana-config.yaml`. CI also runs a Docker Compose smoke job that exercises chat ingest → sentiment → GraphQL end to end. CI runs on pull requests, on pushes to `main`, and on manual dispatch, and a `changes` job path-filters the rest: only the Java services whose directories changed are built, and a change under `.github/workflows/` or `config-server/config-repo/` runs everything. Actions are pinned by commit SHA with the version in a comment; bump the SHA and the comment together. CI also lints every Dockerfile with hadolint (`.hadolint.yaml`), runs a Trivy filesystem scan (dependencies, Dockerfiles, Compose, Kubernetes, secrets; fails on HIGH/CRITICAL with a fix; suppressions with a reason in `.trivyignore`) and uploads it to code scanning, and publishes a CycloneDX SBOM artifact on pushes to `main`. After every check passes on a push to `main` (or a manual dispatch of any ref), `publish-images` builds the ten service images and pushes them to GitHub Container Registry as `ghcr.io/8wali8/streamsense/<service>:<sha>` (plus `:main` when the ref is `main`); deployments pull those tags rather than building (`docs/planning/cloud-hosting.md`). Renovate (`renovate.json`) opens dependency PRs weekly: digest and patch bumps auto-merge on green CI, Spring platform and major runtime-image bumps wait for a person. `pre-commit install` runs ruff, Prettier, actionlint, hadolint, and the whitespace/YAML/JSON/private-key hooks locally (`.pre-commit-config.yaml`); `CONTRIBUTING.md`, `SECURITY.md`, `.github/CODEOWNERS`, and the PR and issue templates set the expectations for changes.

### Hosted demo (GCP VM)

`terraform/` holds the GCP resources for the demo VM (VPC, firewall, static address, service account, one shielded `e2-standard-8` whose startup script installs Docker and clones the repository into `/opt/streamsense`); `docs/planning/cloud-hosting.md` has the decisions and `docs/hosting.md` the runbook. State is local and git-ignored, the provider lock file is committed. After touching the module run `terraform fmt -recursive`, `terraform init -backend=false && terraform validate`, and `tflint --chdir terraform`; CI runs the same in `terraform-checks`, and the Trivy misconfiguration gate scans the files too (suppress a finding inline with `#trivy:ignore:<id>` and a reason, as `main.tf` does for the public address). Keep every new action pinned by SHA and every provider by version. On the VM, `tools/deploy/deploy.sh` (linked as `streamsense-deploy`, run with sudo) checks out the ref matching `STREAMSENSE_IMAGE_TAG` (shell, else the env file, else `main`), refreshes secrets, pulls the images, starts the overlay with the Twitch env from `/etc/streamsense/twitch.env` (template in `tools/deploy/twitch.env.example`; `STREAMSENSE_DOMAIN` there switches Caddy to HTTPS at that name, after the script has checked the name resolves to the VM), waits for health, verifies the console and the gateway through Caddy (certificate, redirect, auth, and that only 22, 80, and 443 listen publicly), and prints the console URL; `verify`, `token` (an operator bearer token for a script, since people sign in with Twitch), and `status` are its other subcommands. Keep it shellcheck-clean.

### Kubernetes (kind cluster)

See `docs/kubernetes-kind.md` for cluster setup. Manifests are under `k8s/`; the entry point is the root `kustomization.yaml` (`kubectl kustomize .`). Every container declares `resources` (requests and a memory limit) and a non-root `securityContext` that drops all capabilities and sets `readOnlyRootFilesystem: true`, and every pod sets `runAsNonRoot` and the `RuntimeDefault` seccomp profile at pod level; the paths an image must write get an `emptyDir` named `scratch-<path>` (`/tmp` for the JVM and Python services, plus `/var/run/postgresql` for Postgres and `/etc/kafka` and `/var/log/kafka` for Kafka), proven by running the image with `docker run --read-only --tmpfs <path>` before adding it. Spring services probe `/actuator/health/liveness` and `/readiness` (readiness includes the datastore checks, liveness never does); stateful data lives on PersistentVolumeClaims in `k8s/platform/storage.yaml`, never `emptyDir`. New workloads follow the same shape or the namespace's Pod Security admission will warn. The namespace is default-deny for ingress and egress (`k8s/network/network-policies.yaml`): each workload has one policy listing exactly the peers it needs (plus cluster DNS; chat-service, video-capture-service, and ml-engine may reach public addresses on the ports Twitch and Hugging Face use), and `tools/k8s/check_network_policies.py` (run in CI) derives every dependency from the manifests, config-repo, Prometheus targets, nginx proxies, and Ingress backends and fails when a policy does not allow it, so a new dependency means a new rule in the same change. Policies select pods by their `app` label, so every pod template carries one. Enforcement needs a CNI that implements NetworkPolicy; kind's default does not (see the kind guide).
...
| chat-service | 8081 | Java | Twitch IRC + manual chat ingest → Kafka producer; joins and parts one channel at a time; replays a recording's chat for a VOD import |
...
| analytics-service | 8085 | Java | Aggregates stream metrics from event streams; owns stream sessions (Twitch Helix poller, capture-derived, or imported from a recording), deals, VOD imports and their state (`vod_imports`: the two replay services keep theirs in memory; a stop is fanned out from here and a resume continues from the recorded offset), and the session and deal report summaries |
| video-capture-service | 8090 | Python | Twitch frame capture → MinIO, transcript audio → ml-engine; one worker per channel, started and stopped one at a time; replays a recording with its original timestamps for a VOD import (`vod_import.py`) |
...
**Downstream URLs are required and every outbound call is bounded.** analytics-service calls sentiment-service to point relevance at a new deal's sponsor, and chat-service and video-capture-service to replay a recording for a VOD import (`streamsense.services.*`, each bounded, best effort, and absent when no base URL is set). The gateway binds `streamsense.services.*` into a validated `DownstreamServicesProperties` (all six base URLs `@NotBlank`, plus `connect-timeout` and `response-timeout` applied to every `WebClient` through a `WebClientCustomizer`); proxied routes use `spring.cloud.gateway.server.webflux.httpclient.*`. sentiment-service and video-service bind their `RestTemplate` from `streamsense.ml.*`. Never add a `localhost` default to a service URL and never build an HTTP client without a timeout.
...
- **Auth** — JWT validation filter (`config/GatewayAuthWebFilter`). Sign in with Twitch lives in `auth/` (`TwitchSignInService`, `OAuthStateCodec` for the signed state cookie, `TwitchIdentityClient`, `GatewayTokenIssuer`) behind `web/TwitchSignInController` (`GET /auth/providers`, `/auth/twitch/login`, `/auth/twitch/callback`): the gateway is the OAuth client, learns the login from `id.twitch.tv/oauth2/validate`, and mints its own HS256 token with `login` and `role` claims, delivered to the console in the URL fragment (`#token=<jwt>`, captured on load; `streamsense.gateway.auth.twitch.*`, operators in `streamsense.gateway.auth.operators`). A token whose `role` is `streamer` is refused (403 `operator_required`) for anything but a read on the operator-only paths (`/api/chat/**`, `/api/video/**`, `/api/sentiment/**`, `/ml/**`), and is confined to its own channel: `auth/AuthScope` carries the login and role, the auth filter refuses REST paths and `streamer` parameters naming another channel (`channel_forbidden`), `config/AuthScopeHeadersFilter` forwards the scope to the services as `X-StreamSense-Auth-Login` and `-Role` (client copies dropped), `graphql/ChannelScopeInterceptor` refuses any top-level field whose `streamer` argument names another channel (`CHANNEL_FORBIDDEN`, queries and subscriptions), and the by-id resolvers (`deal`, `dealSummary`, `session`, `sessionSummary`, `sponsorMoments`) check the fetched owner; analytics-service's `web/ChannelScopeFilter` applies the same rule from the headers to its by-id routes, the deals list, and deal creation, and refuses a deal's deletion to a streamer (operators only; a deal is edited with `PUT`, ended by setting its end date, and deleted only once unshared, see `docs/contracts/deals.md`). Carved out of those operator-only paths are the self-service paths (`streamsense.gateway.auth.self-service-paths`: `/api/chat/twitch/channels/*`, `/api/video/capture/channels/*`), where a streamer starts and stops measurement of their own channel: the channel is the last path segment, so the scope check refuses anyone else's, and a self-service path naming no channel is refused rather than treated as unscoped. Every accepted token carries a role: `auth/JwtAuthTokenValidator` refuses one whose `role` is missing or not `operator`/`streamer` with 401 `missing_role`, so a scope is either the operator or a streamer confined to one channel (`tools/mint-jwt.py --role …` mints either shape by hand for a script; the access links of old, tokens without a role, were retired in `auth/06`). A GraphQL POST carrying `X-StreamSense-Share` instead of a bearer token enters read-only share mode, where `graphql/ShareLinkInterceptor` resolves the token to its deal through analytics-service and allows only `deal`, `dealSummary`, `sessionSummary`, and `sponsorMoments` for that deal, without the fee (see `docs/contracts/deals.md`)
...
In Docker, an unprivileged nginx (`nginxinc/nginx-unprivileged`, uid 101, port 8080 inside the container, published as `http://localhost:3000`) serves the built console with gzip, immutable caching for `/assets/`, `no-cache` for the HTML shell, and the security headers in `frontend/streamsense-headers.conf` (CSP, `X-Frame-Options`, `nosniff`, referrer and permissions policies). It proxies `/graphql` (with WebSocket upgrade), `/api/`, and `/auth/` (sign-in) to the gateway and only `POST /ml/segment` to ml-engine; every other `/ml/` path is a 404. `/healthz` is the container's own health endpoint. Kubernetes runs the same image from `k8s/apps/frontend.yaml` behind the `streamsense.local` ingress. `npm run dev` (port 5173, so it can run next to the Compose frontend on 3000) proxies the same three routes to the Compose backend (`VITE_DEV_API_TARGET` overrides the target; `/ml/segment` is a gateway route), so a running `make up` is all it needs. REST calls go through `src/lib/api-client.ts` (base URL from `src/config/env.ts`, the same bearer token as GraphQL, JSON, a timeout on every call, `ApiError` carrying the service's RFC 9457 problem details) via one typed function per endpoint in `src/api/<feature>.ts`; components never call `fetch` or read `import.meta.env` directly. Periodic REST reads use `usePolledResource`.

Layout: `src/App.tsx` is the router plus the sign-in gate (`react-router`; `AppRoutes` is mounted under a `BrowserRouter` in production and a `MemoryRouter` in tests). `features/access/AccessGate` renders `LoginPage` (the Twitch button when `GET /auth/providers` says so, a sentence saying sign-in is off when it does not, the failure sentence from `?signin=<reason>`, and the demo link; there is no field to paste a token into) instead of the app until local storage holds a bearer token that has not expired (`src/lib/auth-token.ts`: the token a Twitch sign-in comes back with in the `#token=<jwt>` fragment is captured on load and stripped from the address bar; `AccessPanel` in the sidebar shows the Twitch login and role and signs out; the Operations link and route render only for an operator sign-in (`isOperatorView` in `src/lib/view-as.ts`: `isOperatorSession` is true for the operator role or no token at all), and `readStoredSelection` pins a streamer to their own channel. An operator can **view as** a streamer from the access panel: `view-as.ts` keeps the login in session storage for the tab, the console pins to that channel and hides the operator controls until "Back to my view", and the token is unchanged, so it is the streamer's shape of the console over the operator's access, for walking the streamer's path without a second account); a share-link tab passes the gate without a bearer token. `/demo` is the public demo: the same `AppRoutes` mounted under a router with `basename="/demo"`, no gate, running on the sealed snapshot in `src/demo/` (`mode.ts` decides demo mode from the path once per load and `openDemoLanding` rewrites a bare `/demo` to `sessions/latest?sponsor=<the snapshot's sponsor>`, so the demo opens on the newest report rather than a live console with nothing in it, and the home page stays reachable from the sidebar; `demo-link.ts` is the Apollo link that answers queries from `snapshot.json` and keeps subscriptions quiet; `api-client.ts` answers REST reads from it and refuses writes; `DemoChrome.tsx` is the banner with the deal and latest-report actions and the sidebar's sign-in call to action; the live console, the write controls, and the operations page are not rendered: the demo is past streams). The snapshot is exported by `tools/demo/export_snapshot.py` from a stack with good data and committed; the demo never sends a request to the gateway (`docs/planning/demo-page.md`). Routes: `/` is the streamer's home (`features/home/`: the live strip over the console, which renders only while the channel is live (offline, the strip points at the last report), the deals panel from `features/deals/` with its new-deal form (`DealForm.tsx`, which also edits a deal on its page: sponsor, dates, and promised streams up front, the fee and the other terms behind a "More settings" disclosure, closed for a new deal; amounts are labelled USD; an edit sends the whole deal back), and the history with its track record in `track-record.ts`; the general chat and transcript sit in a closed drawer under the player), `/deals/:dealId` is the deal page (`features/deals/`: totals against the fee, the per-stream trend in `trend.ts`, the streams inside the deal, the share control, the owner's Edit and "End today" controls plus an operator's Delete (asks first, refused while shared), and the owner's list of the channel's Twitch recordings to import in `ImportStreams.tsx` with its pure helpers in `import-streams.ts` (each row shows the import's state as analytics-service records it, polled from `GET .../vods/imports`, with Stop while it runs and Resume after a stop or a failure); formatting in `deal-format.ts`), `/sessions/:sessionId` is the one-page session report with its detail pages `/value`, `/mentions`, `/risk`, and `/stream` beneath it and `/sessions/latest` redirecting to the current channel's newest session, carrying its query (so `?sponsor=X` survives the redirect) (`features/session/`: the page, the timeline layout in `timeline.ts`, formatting and VOD links in `report-format.ts`), `/ops` is the operator-only page (`features/ops/`: pipeline status, channel control, the sponsor relevance profile editor, the metrics overview, raw sentiment and detection events, and the segmentation preview). Nothing diagnostic (event ids, model versions, box coordinates, health pills, the streamer form) renders outside `/ops`. The shell (`src/components/AppShell.tsx`) is the left navigation plus the routed outlet. A tab opened from a share link (`?share=<token>`, kept in session storage by `src/lib/share-token.ts` and sent as the share header by `authHeaders` when there is no bearer token) renders the shell without navigation and routes only the deal and session pages. Feature code lives under `src/features/<feature>/` (`capture/` is the home page's measurement control, which starts and stops chat ingest and video capture for the channel being viewed through the per-channel routes, so a streamer needs no operator; `console/` is the live player plus transcript, sponsor, and chat feeds; `streamer/` is the streamer/sponsor selection and its runtime switching, provided once by `StreamerProvider` and read anywhere with `useStreamer()`; the selection persists in `localStorage` under `streamsense.selection`; `status/` is the health and ingest pills). `src/components/` holds only building blocks without a feature (`AppShell`, `MetricCard`, `ErrorBoundary`), pure helpers go in `src/lib/`, hooks in `src/hooks/`. Styling is one theme in `src/index.css` (graphite grounds, the on-air red accent only for live state and the primary action, teal for the sponsor being on screen, green/amber/red only for sentiment and risk); IBM Plex is bundled from `@fontsource` so the nginx CSP's `font-src 'self'` holds. Every error a user sees goes through `describeError` (`src/lib/errors.ts`): the REST problem `detail` when the service sent one, the gateway's `extensions.code` translated to words (`DOWNSTREAM_UNAVAILABLE` becomes "<host> is unavailable"), a 401 from either transport as "this console needs a sign-in", a REST 403 as "your account is not allowed to change this", otherwise the raw message; never render `error.message` directly. Any panel that shows a history query with live subscription events on top uses `useLiveFeed` (or `useLiveEvents` when there is no history query): it de-duplicates by id, keeps the newest first, and resets when the streamer changes, so never hand-roll a `useSubscription` + `useState` buffer. Pure list-building logic (for example `features/console/transcript-lines.ts`) is a plain function with a unit test, not JSX. Each major section of the page is wrapped in `ErrorBoundary` so one failing panel cannot blank the console. Do not add a component nobody renders; delete it.

Tests: every component test renders with a real Apollo client (`renderWithApollo` in `src/test/apollo.tsx`) and answers HTTP with MSW handlers (`src/test/msw.ts` helpers: `graphqlData`, `graphqlError`, `graphqlPending`, `restJson`, `restProblem`); subscriptions are driven with `emitSubscription`. Never `vi.mock` `@apollo/client/react`, `fetch`, or a sibling component: mock the network at the edge, not the module graph. Fixtures in `src/test/fixtures.ts` carry `__typename` because the cache needs it. Interact through `@testing-library/user-event`. Unhandled requests fail the test. ESLint is type-aware (`recommendedTypeChecked`) with `jsx-a11y`; Prettier is the formatter (`.prettierrc.json`, 120 columns); `vitest run --coverage` enforces the floors in `vite.config.ts`, so a change that drops coverage below them fails CI. Since 2026-09-09 the redesign line keeps tests minimal (core and non-obvious logic only, no tests that only restate markup) and the floors were lowered to match; a later pass deepens the tests and raises the floors.
...
- `docs/contracts/` — GraphQL API contracts (`sessions.md` for stream sessions and the session report: HELIX vs CAPTURE sources, `sessionSummary` with its value and response arithmetic, `sponsorMoments`, and the session or range arguments the analytics queries accept; `deals.md` for deals, their summaries, how a session finds its deal, and the persisted sponsor relevance profiles)
