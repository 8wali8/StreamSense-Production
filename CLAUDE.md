...
| chat-service | 8081 | Java | Twitch IRC + manual chat ingest → Kafka producer; replays a recording's chat for a VOD import |
...
| video-capture-service | 8090 | Python | Twitch frame capture → MinIO, transcript audio → ml-engine; replays a recording with its original timestamps for a VOD import (`vod_import.py`) |
