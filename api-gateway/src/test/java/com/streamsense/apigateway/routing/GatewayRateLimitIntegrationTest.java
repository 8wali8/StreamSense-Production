...
class GatewayRateLimitIntegrationTest {
    ...
    static void registerProperties(DynamicPropertyRegistry registry) {
        ...
        registry.add("spring.cloud.gateway.server.webflux.routes[0].uri", () -> CHAT_SERVICE.url("/").toString());
    ...
    void rejectsRequestsAfterConfiguredBurstLimit() {
        ...
        postIngest("203.0.113.10")
                ...
                .jsonPath("$.type").isEqualTo("https://streamsense.dev/problems/rate-limited")
                .jsonPath("$.status").isEqualTo(429)
                .jsonPath("$.instance").isEqualTo("/api/chat/ingest")
                .jsonPath("$.service").isEqualTo("api-gateway")
                .jsonPath("$.correlationId").isNotEmpty();

        assertThat(meterRegistry.get("streamsense_gateway_rate_limit_rejections_total")
    ...
    void tracksClientsSeparately() {
        CHAT_SERVICE.enqueue(new MockResponse().addHeader("Content-Type", "application/json").setBody("{\"eventId\":\"evt-a\"}"));
        CHAT_SERVICE.enqueue(new MockResponse().addHeader("Content-Type", "application/json").setBody("{\"eventId\":\"evt-b\"}"));
    ...
    void keysOnTheEntryAppendedByTheTrustedProxy() {
        CHAT_SERVICE.enqueue(new MockResponse().addHeader("Content-Type", "application/json").setBody("{\"eventId\":\"evt-c\"}"));
        CHAT_SERVICE.enqueue(new MockResponse().addHeader("Content-Type", "application/json").setBody("{\"eventId\":\"evt-d\"}"));
        CHAT_SERVICE.enqueue(new MockResponse().addHeader("Content-Type", "application/json").setBody("{\"eventId\":\"evt-e\"}"));

        // The leftmost entry is client-supplied; only the rightmost one came from the trusted hop.
        postIngest("10.0.0.1, 203.0.113.50").expectStatus().isOk().expectHeader().valueEquals("X-RateLimit-Remaining", "1");
        postIngest("10.0.0.2, 203.0.113.50").expectStatus().isOk().expectHeader().valueEquals("X-RateLimit-Remaining", "0");
        postIngest("10.0.0.2, 203.0.113.51").expectStatus().isOk().expectHeader().valueEquals("X-RateLimit-Remaining", "1");
    }
