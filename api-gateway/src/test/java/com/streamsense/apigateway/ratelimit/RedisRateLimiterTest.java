...
class RedisRateLimiterTest {
    ...
    static final GenericContainer<?> REDIS = new GenericContainer<>(
            DockerImageName.parse("redis:7.4.11-alpine@sha256:ff02b58f971e7d7d156a1267e283fcbbeee91773b6aa36c49dac28ecfe28eadf"))
    ...
    void failsOpenOrClosedWhenRedisIsUnreachable() {
        ...
        try {
            RedisRateLimiter open = new RedisRateLimiter(brokenRedis, fixedClock("2026-04-11T12:00:00Z"), true, registry);
            RedisRateLimiter closed = new RedisRateLimiter(brokenRedis, fixedClock("2026-04-11T12:00:00Z"), false, registry);
            ...
            assertThat(registry.get("streamsense_gateway_rate_limit_store_errors_total").counter().count()).isEqualTo(2.0d);
        } finally {
            unreachable.destroy();
        }
    }
    ...
    private static LettuceConnectionFactory factoryFor(String host, int port) {
        LettuceClientConfiguration clientConfiguration = LettuceClientConfiguration.builder()
                ...
                .build();
        LettuceConnectionFactory factory = new LettuceConnectionFactory(new RedisStandaloneConfiguration(host, port), clientConfiguration);
