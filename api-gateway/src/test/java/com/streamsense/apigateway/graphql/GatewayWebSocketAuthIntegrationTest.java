...
class GatewayWebSocketAuthIntegrationTest {
    ...
    void closesTheSocketWhenTheTokenIsSignedWithAnotherKey() {
        ...
        Session session =
                GraphqlTransportWsProbe.connect(port, Map.of("Authorization", "Bearer " + forged), false, TIMEOUT);
    ...
    void closesAnAcknowledgedSocketOnceItsTokenExpires() {
        String shortLived = TestJwtTokens.token(
                "demo-user",
                "streamsense-local",
                List.of("streamsense-clients"),
                Instant.now().plusSeconds(2).getEpochSecond(),
                null,
                "HS256");

        // No query and no client-side close: the session only ends if the server ends it at exp.
        Session session =
                GraphqlTransportWsProbe.connect(port, Map.of("Authorization", "Bearer " + shortLived), false, TIMEOUT);
