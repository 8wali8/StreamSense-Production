...
public class GatewayAuthWebFilter implements WebFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        ...
        if (result.valid()) {
            ...
            // their own deals, but not look at another channel. Routes that name the channel by id are checked
            // by analytics-service from the scope headers; GraphQL by ChannelScopeInterceptor and the resolvers.
            if (!scope.isOperator()
