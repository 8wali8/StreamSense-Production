...
public class SentimentServiceClient {
    ...
    public SentimentServiceClient(WebClient.Builder webClientBuilder, DownstreamServicesProperties services) {
        this.webClient = webClientBuilder
                .baseUrl(services.getSentimentService().getBaseUrl())
                .build();
    }
    ...
    public Mono<List<SentimentAnalysisEvent>> recentSentiment(String streamer, int limit) {
        return webClient
                ...
                .doOnSubscribe(
                        subscription -> log.info("fetching recent sentiment streamer={} limit={}", streamer, limit))
                .doOnSuccess(response -> log.info(
                        "received recent sentiment streamer={} count={}",
                        streamer,
        ...
        return webClient
                ...
                .uri(uriBuilder -> {
                    var builder = uriBuilder
                            .path("/api/sentiment/sponsor/recent")
                            .queryParam("streamer", streamer)
                            .queryParam("limit", limit);
                ...
                .doOnSubscribe(subscription -> log.info(
                        "fetching recent sponsor sentiment streamer={} sponsor={} limit={}", streamer, sponsor, limit))
                .doOnSuccess(response -> log.info(
                        "received recent sponsor sentiment streamer={} sponsor={} count={}",
                        streamer,
                        sponsor,
                        response != null ? response.size() : 0));
    }
    ...
    public Mono<List<TranscriptSegmentEvent>> recentTranscriptSegments(String streamer, int limit) {
        return webClient
                .get()
                .uri(uriBuilder -> uriBuilder
                        .path("/api/sentiment/transcript/recent")
                        .queryParam("streamer", streamer)
                        .queryParam("limit", limit)
                        .build())
                .retrieve()
                ...
                .doOnSubscribe(
                        subscription -> log.info("fetching recent transcript streamer={} limit={}", streamer, limit))
                .doOnSuccess(response -> log.info(
                        "received recent transcript streamer={} count={}",
                        streamer,
                        response != null ? response.size() : 0));
    }
    ...
    public Mono<List<TranscriptSentimentEvent>> recentTranscriptSentiment(String streamer, int limit) {
        return webClient
                .get()
                .uri(uriBuilder -> uriBuilder
                        .path("/api/sentiment/transcript/sentiment/recent")
                        .queryParam("streamer", streamer)
                        .queryParam("limit", limit)
                        .build())
                .retrieve()
                ...
                .doOnSubscribe(subscription ->
                        log.info("fetching recent transcript sentiment streamer={} limit={}", streamer, limit))
                .doOnSuccess(response -> log.info(
                        "received recent transcript sentiment streamer={} count={}",
                        streamer,
                        response != null ? response.size() : 0));
    }

    public Mono<List<TranscriptSentimentEvent>> recentSponsorTranscriptSentiment(
            String streamer, String sponsor, int limit) {
        return webClient
                .get()
                .uri(uriBuilder -> {
                    var builder = uriBuilder
                            .path("/api/sentiment/transcript/sponsor/recent")
                            .queryParam("streamer", streamer)
                            .queryParam("limit", limit);
                    if (sponsor != null && !sponsor.isBlank()) {
                        builder.queryParam("sponsor", sponsor);
