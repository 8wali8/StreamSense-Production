...
function dealsLead(home: HomeSponsor, count: number, loading: boolean): string {
...
export function DealsPanel({ streamer, deals, loading, error, home, onCreated }: DealsPanelProps) {
  ...
  return (
    <section className="panel deals" aria-label="Deals">
      <div className="panel-heading deals-heading">
        <div>
          <h2>Deals</h2>
          <p>{dealsLead(home, deals.length, loading)}</p>
        </div>
        {!creating && (
